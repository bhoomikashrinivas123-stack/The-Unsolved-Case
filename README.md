...
<body>
    ...
    <div class="sidebar">
      ...
      <div class="sidebar-section">
        ...
        <div class="evidence-list" id="suspectList">
          <div class="evidence-item">🔴 Alex - Diamond Dealer</div>
          <div class="evidence-item">🔵 Ben - Security Guard</div>
          <div class="evidence-item">🟡 Clara - Museum Curator</div>
        </div>
      </div>
    </div>
  </div>
  ...
  <script>
    ...
    const rooms = [
      {
        ...
        visualClues: [
          { emoji: "📝", label: "Note Found", description: "A handwritten note: 'Meet at 1_:3_' — some digits are rubbed out and hard to read." },
          { emoji: "🕐", label: "Clock", description: "The museum clock shows 3:30 PM - exactly when the alarm triggered" },
          { emoji: "🔒", label: "Safe Lock", description: "High-security safe requires 4-digit code - only opened during the gap" },
          { emoji: "📊", label: "Security Log", description: "Digital record showing visitor access times" }
        ],
        puzzle: {
          question: "What time was the diamond stolen? (Use format: HH:MM)",
          answer: "15:30",
          hint: "Some digits are hard to read on the note — a forensics tool might reveal faint ink. Think about what could show hidden writing."
        },
      ...
      },
      {
        ...
        clues: [
          { type: "text", content: "🗣 Alex: 'I was in my shop the whole afternoon'" },
          { type: "text", content: "🗣 Ben: 'I was on duty at the north entrance'" },
          { type: "text", content: "🗣 Clara: 'I was in my office reviewing documents'" },
          { type: "text", content: "📱 Phone records show Clara made a call at 3:15 PM" }
        ],
        visualClues: [
          { emoji: "👨", label: "Alex", description: "Diamond dealer - calm behind the glass; a shadow in the display suggests someone left around 3:15." },
          { emoji: "👮", label: "Ben", description: "Security guard - uniform and patrol watch visible; the watch face seems to show a different time than the camera timestamp." },
          { emoji: "👩", label: "Clara", description: "Museum curator - concerned and defensive; phone records show a call at 3:15, but it's marked as 'forwarded' from an external number." },
          { emoji: "📹", label: "Security Camera", description: "Obscured footage: a blurred figure in dark clothing enters wearing a jacket similar to Ben's, face indistinct (hard to be certain)." }
        ],
        puzzle: {
          question: "Who is lying about their alibi? (Enter: Alex, Ben, or Clara)",
          answer: "clara",
          hint: "Some evidence looks definitive, but one clue seems staged: the call is forwarded and the footage is blurred. Reconcile the forwarded call with the camera — whose story still contradicts the records?"
        },
      ...
      },
      {
        ...
        clues: [
          { type: "text", content: "🔢 The code is 4 digits long" },
          { type: "text", content: "📌 First clue: '15' (from the time of theft)" },
          { type: "text", content: "📌 Second clue: '30' (from the note)" },
          { type: "text", content: "🎯 Combine the clues to form the final code" }
        ],
        visualClues: [
          { emoji: "🏛", label: "First Digit", description: "Number 1 - A single pillar in the museum hall (represents 1)" },
          { emoji: "⭐", label: "Second Digit", description: "Number 5 - Five-pointed star ornament from the display (represents 5)" },
          { emoji: "🔺", label: "Third Digit", description: "Number 3 - Three triangular security barriers (represents 3)" },
          { emoji: "🔐", label: "Fourth Digit", description: "Number 0 - A perfectly round vault padlock (represents 0)" }
        ],
        puzzle: {
    ...
    function loadRoom(roomIndex) {
      ...
      content.innerHTML = `
        <div class="timer" id="timerDisplay">⏱ ${formatTime(gameState.timeRemaining)}</div>
        <div class="clues">
          <h3>📖 Story</h3>
        ...
        <div class="puzzle-section">
          ...
          <button class="hint-btn" id="hintBtn" onclick="useHint()" ${gameState.roomTimeExpired ? 'disabled' : ''}>💡 Use Hint (-30s)</button>
          <div id="message"></div>
        </div>
      `;
        ...
        if (timerDisplay) {
          ...
          if (gameState.timeRemaining <= 0) {
            ...
            if (messageEl) {
              ...
              messageEl.textContent = "❌ Time's up! Moving to next room...";
            }
    ...
    function submitAnswer() {
      ...
      if (gameState.roomTimeExpired) {
        if (messageEl) {
          messageEl.className = "message error";
          messageEl.textContent = "⏱ You can't answer — time expired.";
        }
        return;
      }
      ...
      if (raw === room.puzzle.answer.toLowerCase()) {
        ...
        if (messageEl) {
          messageEl.className = "message error";
          messageEl.textContent = "❌ Incorrect — controls locked for 15s. (-100 points)";
        }
        updateUI();
        ...
        gameState.cooldownInterval = setInterval(() => {
          ...
          if (gameState.cooldownRemaining <= 0) {
            ...
            if (cooldownMsgEl) {
              ...
              cooldownMsgEl.textContent = ❌ Incorrect — try again in ${gameState.cooldownRemaining}s;
            }
          }
        }, 1000);
      }
    ...
    function useHint() {
      ...
      // No score penalty for hints anymore — only reduce time by 30s
      gameState.hintsUsed++;
      ...
      if (gameState.timeRemaining <= 0) {
        ...
        messageEl.className = "message error";
        messageEl.textContent = "💡 Hint used — but it consumed the remaining time. Moving to next room...";
        updateUI();
        setTimeout(() => {
          gameState.currentRoom++;
          loadRoom(gameState.currentRoom);
      ...
      messageEl.textContent = 💡 Hint: ${room.puzzle.hint} (-30s);
      updateUI();
    }
    ...
    function checkRiddleAnswer() {
      ...
      if (accepted.includes(raw)) {
        ...
        msg.textContent = '✅ Correct! This is the Internet Lab.';
        input.disabled = true;
